...
def tide_behav_extr(df, tagno,
                    tideFL=10, tideLV=[0.5, 0.7, 0.5],
                    behavFL=16, behavLV=[0.5, 0.7, 0.5],
                    dt=10):
    """
    ...
    time = (df['time'] - df['time'].iloc[0]).dt.total_seconds() / 3600.0 / 24.0  # en jours
    ...
    tide_found = (rmse_tide < tideLV[0]) & (rsq_tide > tideLV[1]) & (ampli_tide > tideLV[2])
    ...
    behav_found = (rmse_behav < behavLV[0]) & (rsq_behav > behavLV[1]) & (ampli_behav > behavLV[2])
...
def datalikelihood_fast_xr_multi_vec_mask(td_depth, td_time, tidal_ds, sigma_tid=2.0):
    ...
    for t in tqdm(range(n_times), desc="Temps"):
        ...
        LIK[t, :, :] = np.exp(-0.5 * ((td_mean - mu_eff)/sigma_tid)**2) / (sigma_tid * np.sqrt(2*np.pi))
...
def datalikelihood_full_xr_vec(td_depth, td_time, tidal_ds, sigma_tid=2.0):
    ...
    tidal_pred = amp_exp * np.cos(omega_exp * td_time_exp + phase_exp)  # (n_times, n_const, n_lon)
    tidal_sum = tidal_pred.sum(axis=1)                                  # (n_times, n_lon)
    mu = -tidal_sum[:, None, :] + depth_mean[None, :, :]                # (n_times, n_lat, n_lon)
    ...
    for t in tqdm(range(n_times), desc="Temps"):
        ...
        LIK_t = np.exp(-0.5 * ((x_mean - mu_ij)/sigma_vec)**2) / (sigma_vec * np.sqrt(2*np.pi))
...
def datalikelihood_tide_only_normalized(td_depth, td_time, tidal_ds, tide_found, sigma_tid=2.0):
    """
...
def datalikelihood_tide_only_full_normalized(td_depth, td_time, tidal_ds, tide_found, sigma_tid=2.0):
    """
    Calcule une PDF normalisée pour chaque instant.
    - Si marée détectée : PDF = vraisemblance (gaussienne)
    - Sinon : PDF uniforme sur l'océan (somme = 1)
...
###
        mu_ij = mu[t, :, :]    # shape (n_lat, n_lon)
        x_mean = np.nanmean(td_depth[t, :])
        L = np.exp(-0.5 * ((x_mean - mu_ij)/sigma_vec)**2) / (sigma_vec * np.sqrt(2*np.pi))
###
        ###L = np.exp(-0.5 * ((x_mean - mu_eff)/sigma_tid)**2)
        L = L * mask  # appliquer masque
