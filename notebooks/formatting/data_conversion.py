...
def create_metadata_file(file_path,destination_path):
    ...
    metadata = {"pit_tag_id": tag_id, "scientific_name": "Dicentrarchus labrax", "common_name": "European seabass", "project": "BARGIP"}
    print()
...
def convert_to_utc_with_formatting(date, time_zone):
    ...
    possible_formats = ["%d/%m/%Y %H:%M", "%d/%m/%y %H:%M", "%d/%m/%Y %H:%M:%S",
...
def format_date(date):
    ...
    """
    possible_formats = ["%d/%m/%Y %H:%M", "%d/%m/%y %H:%M", "%d/%m/%Y %H:%M:%S", "%d/%m/%y %H:%M:%S","%y-%m-%d %H:%M:%S","%Y-%m-%d %H:%M:%S"]
...
def extract_tagging_events(file_path):
    ...
    with open(file_path, newline='', encoding='latin-1') as csvfile:
        ...
        data = {"event_name" : ["release","fish_death"], "time" : dates , "longitude" : lon, "latitude" : lat}
        ...
        for line in csv_reader:
            ...
            if not reached_target_line:
                ...
                if not line:
                    ...
                    line[0] = convert_to_utc_with_formatting(line[0],time_zone) #Formating date to ISO8601 format and convert from CEST to UTC                        
                    line[1] = np.float64(line[1])  # Changing data type from str to float 64
                    line[2] = np.float64(line[2])  # Changing data type from str to float 64
                    
                    data.append(line)
    
    # Convert all the data into a pandas DataFrame
    df = pd.DataFrame(all_data, columns=['time', 'pressure', 'temperature'])[["time","temperature","pressure"]]
